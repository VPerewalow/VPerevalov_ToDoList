...
function App() {
  ...
  const filteredTodos = todos.todo.filter((todo: ITodo) =>
    todo.text && todo.text.toLowerCase().includes(todos.searchQuery.toLowerCase()));

  return (
    <>
      <div className='container'>
        <Header loadTodo={loadTodo} inputText={inputText} setInputText={setInputText} addTodo={addTodo}
          deleteAllTodo={() => dispatch(deleteAllTodoRedux())} deleteLastTodo={deleteLastTodo}
          searchTodo={filterTodos}></Header>
        <div className='counters'>
          <p>Total: {todos.totalTodos}</p>
          <p>Completed: {todos.completedTodos}</p>
          <input type="text" placeholder="Search todos..." className='header__input' onChange={(e) => filterTodos(e.target.value)} />
        </div>
